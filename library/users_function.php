...
function validate_form($password,$email,$mobile) {

 $Errors = array();
          if (empty($password) && $password != '-') {
               $Errors[] = "password is required";
            }else {
               $name = test_input($password);
            }
			...
			if (empty($mobile)) {
			   ...
			   if(preg_match("/^[0-9]*$/", $mobile )) {
			     ...
			     $Errors[] = "Mobile no. format is not matched";
			   }
            }
...
function check_empty($name,$fieldname){
if(empty($name)){
...
//
function current_oldpass($currentpassword,$oldpassword) {
...
function password_confirm($currentpassword,$oldpassword) {
