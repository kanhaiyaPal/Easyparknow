...
<?php
	...
	if(isset($_POST['reset_password']) && ($_POST['reset_password']=='Change') && (isset($_POST['csrf_token'])) && ($_SESSION['crsf_protect_token_dash'] == $_POST['csrf_token']))
	{
		...
		if($user['id']){
			if($_POST['old_mail'] != $_SESSION['adminlogged']['email'])
			{
				$data = array('email' => $_POST['old_mail']);
				$em_id = $db_dashboard->update('users', $data);
			}
			...
			{ 
				unset($_SESSION['adminlogged']);
				generate_alerts_admin_pages("Email Updated!");
				header("Location:index.php?msg=update");
				exit();
			}
			...
			{ 
				unset($_SESSION['adminlogged']);
				generate_alerts_admin_pages("Password Updated!");
				header("Location:index.php?msg=update");
				exit();
			} 
				
			generate_alerts_admin_pages("Error validating form please enter form data again");
		  			...
		  			<div class="row">
		  				<div class="col-md-12">
		  					<div class="content-box-header">
			  					<div class="panel-title">Change Password/E-mail</div>
				  			</div>
				  			<div class="content-box-large box-with-header">
				  				<div class="alert alert-warning">Changing the password/e-mail will log you out from current session</div>
				  				<form method="post" action="" class="form-horizontal" >
				  					<div class="form-group">
				  						<label>E-mail Id</label>
				  						<input class="form-control" type="email" name="old_mail" value="<?=$_SESSION['adminlogged']['email']?>" required="required">
				  					</div>
				  					<div class="form-group">
				  					...
				  					<div class="form-group">
				  						...
				  						<input class="form-control" type="Password" id="password" name="new_pass" minlength="6" >
				  					</div>
				  					<div class="form-group">
				  						<label>Repeat New Password</label>
				  						<input class="form-control" type="Password" id="confirm_password" name="rep_new_pass" minlength="6">
				  					</div>
